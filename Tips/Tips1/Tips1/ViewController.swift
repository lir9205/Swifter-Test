...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        //闭包表达式语法
        let reverseNames = names.sort({(s1: String, s2: String) -> Bool in return s1 > s2})
        //根据上下文推断语法
        let reverseNames1 = names.sort ({(s1, s2) -> Bool in return s1 > s2 })
        //单表达式闭包隐式返回
        let reverseNames2 = names.sort ({(s1, s2) -> Bool in s1 > s2 })
        //参数名称缩写
        let reverseNames3 = names.sort({$0 > $1})
        //运算符方法
        let reverseNames4 = names.sort(>)
        
        
        /*尾随闭包 ：如果你需要将一个很长的闭包表达式作为最后一个参数传递给函数,可以使用尾随闭包来增强函数的可读性。尾
        随闭包是一个书写在函数括号之后的闭包表达式,函数支持将其作为最后一个参数调用*/
        let reverseNames5 = names.sort(){ $0 > $1 }
        //如果闭包表达式是函数或方法的唯一参数,则当你使用尾随闭包时,你甚至可以把 () 省略掉:
        let reverseNames6 = names.sort { $0 > $1 }
...
//        var startLevel = addTwo(2)
        var currentLevel = level ?? addTwo(2)
    ...
    //??的实现
//  ??<T>(optional: T?, @autoclosure defaultValue: () throws -> T) rethrows -> T
    func ww<T>(optional: T?, @autoclosure defaultValue: ()-> T) -> T {
        switch optional {
        case .Some(let value):
            return value
        case .None:
...
//    &&<T : BooleanType>(lhs: T, @autoclosure rhs: () throws -> Bool) rethrows -> Bool
    func ee<T: BooleanType>(lhs: T, @autoclosure rhs: () -> Bool) -> Bool {
        if lhs.boolValue == false {
            return false
        } else if rhs() {
            return true
        } else {
...
//    ||<T : BooleanType>(lhs: T, @autoclosure rhs: () throws -> Bool) rethrows -> Bool
    ...
    func hh<T: BooleanType>(lhs: T, @autoclosure rhs: () -> Bool) -> Bool {
        if lhs.boolValue {
            return true
        } else if rhs() {
            return true
